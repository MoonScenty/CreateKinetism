...
/**
 * Create's {@code BlazeBurnerMovementBehaviour}, drawing with {@link StrayChillerRenderer}.
 *
 * <p>Copied rather than extended because the parts that would need reusing - the head angle and the
 * conductor check - are private, and the one method that differs is the render call at the bottom.
 * On a contraption or a train the chiller turns, smokes and wears a conductor's hat exactly as a
 * Blaze Burner does.</p>
 */
public class StrayChillerMovementBehaviour implements MovementBehaviour {
	...
	public void renderInContraption(MovementContext context, VirtualRenderWorld renderWorld,
		...
		if (!shouldRender(context))
			return;
		StrayChillerRenderer.renderInContraption(context, renderWorld, matrices, buffer, getHeadAngle(context),
