...
/**
 * Create's {@code PressingBehaviour}, forked so the enricher can play its own sound.
 *
 * <p>Create's code is MIT - see LICENSE-THIRD-PARTY.md - and this is a verbatim copy apart from one
 * block. The sound is played inline in {@code tick()} with nothing to override and no hook to pass a
 * different event, so replacing it means owning the class.</p>
 *
 * <p>Everything else is Create's: the 240-tick cycle, the belt/depot/basin/world modes, the
 * speed-scaled stroke rate and the head offset the renderers read.</p>
 */
public class EnrichingBehaviour extends BeltProcessingBehaviour {
	...
	public EnrichingBehaviourSpecifics specifics;
	...
	public interface EnrichingBehaviourSpecifics {
	...
	public <T extends SmartBlockEntity & EnrichingBehaviourSpecifics> EnrichingBehaviour(T be) {
		super(be);
		this.specifics = be;
		mode = Mode.WORLD;
		entityScanCooldown = ENTITY_SCAN;
		whenItemEnters((s, i) -> BeltEnrichingCallbacks.onItemReceived(s, i, this));
		whileItemHeld((s, i) -> BeltEnrichingCallbacks.whenItemHeld(s, i, this));
	}
	...
	public void tick() {
		...
		if (runningTicks == CYCLE / 2 && specifics.getKineticSpeed() != 0) {
			...
			// Create layers two vanilla effects here and swaps to a softer pair over wool. The enricher
			// has a clip of its own, so it just plays that. Pitch still rides the speed: at high RPM the
			// strokes land closer together than the clip is long.
			level.playSound(null, worldPosition, CKSoundEvents.ENRICHING_PROCESS.get(), SoundSource.BLOCKS,
				.75f, .75f + (Math.abs(specifics.getKineticSpeed()) / 1024f));
