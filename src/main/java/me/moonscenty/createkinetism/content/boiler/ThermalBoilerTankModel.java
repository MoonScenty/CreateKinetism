...
/**
 * Same job as the Steel Tank's model wrapper - applies the connected-texture shift across a stack
 * and culls the faces between neighbouring tanks - but for this tank's own local texture set, since
 * it is meant to be reskinned rather than left as a copy of Create's own fluid tank.
 */
public class ThermalBoilerTankModel extends CTModel {

	private static final ModelProperty<CullData> CULL_PROPERTY = new ModelProperty<>();

	public ThermalBoilerTankModel(BakedModel originalModel) {
		super(originalModel, new FluidTankCTBehaviour(CKSpriteShifts.THERMAL_BOILER_TANK,
			CKSpriteShifts.THERMAL_BOILER_TANK_TOP, CKSpriteShifts.THERMAL_BOILER_TANK_INNER));
	}

	public static ThermalBoilerTankModel standard(BakedModel template) {
		return new ThermalBoilerTankModel(template);
	}
