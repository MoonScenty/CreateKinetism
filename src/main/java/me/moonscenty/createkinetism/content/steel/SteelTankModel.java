 ...
 *
 * <p>Two jobs. It applies the connected-texture shifts so a stack of steel tanks reads as one
 * vessel, and it culls the faces between neighbouring tanks so you do not see interior walls
 * through the windows.</p>
 */
public class SteelTankModel extends CTModel {

	private static final ModelProperty<CullData> CULL_PROPERTY = new ModelProperty<>();

	public SteelTankModel(BakedModel originalModel) {
		super(originalModel, new FluidTankCTBehaviour(CKSpriteShifts.STEEL_TANK, CKSpriteShifts.STEEL_TANK_TOP,
			CKSpriteShifts.STEEL_TANK_INNER));
	}

	public static SteelTankModel standard(BakedModel template) {
		return new SteelTankModel(template);
	}
