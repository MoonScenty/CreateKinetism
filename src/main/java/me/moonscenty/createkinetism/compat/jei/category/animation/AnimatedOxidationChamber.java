...
/**
 * The Oxidation Chamber over its basin, for the JEI panel.
 *
 * <p>{@link AnimatedInjectionChamber} without the fluid pass: same housing, same plunge, but nothing
 * is drawn inside it because this machine holds nothing of its own - what it makes goes down into the
 * basin, which draws its own contents.</p>
 */
public class AnimatedOxidationChamber extends AnimatedKinetics {
	...
	public void draw(GuiGraphics graphics, int xOffset, int yOffset) {
		...
		blockElement(CKPartialModels.OXIDATION_CHAMBER_COG).rotateBlock(0, getCurrentAngle() * 2, 0)
			.scale(scale)
			.render(graphics);

		blockElement(CKBlocks.OXIDATION_CHAMBER.getDefaultState()).scale(scale)
			.render(graphics);
		...
		blockElement(CKPartialModels.OXIDATION_CHAMBER_HEAD).atLocal(0, plunge, 0)
			.scale(scale)
			.render(graphics);
		...
		blockElement(CKPartialModels.OXIDATION_CHAMBER_ARROWS).rotateBlock(180, 0, 0)
			.scale(scale)
			.render(graphics);
