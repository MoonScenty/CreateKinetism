...
/**
 * The Mechanical Electrolyzer over its basin, for the JEI panel.
 *
 ...
 * end on end rather than by a cog on the lid, and the electrolyzer's own pole and head instead of the
...
public class AnimatedMechanicalElectrolyzer extends AnimatedKinetics {
	...
	public void draw(GuiGraphics graphics, int xOffset, int yOffset) {
		...
		blockElement(CKBlocks.MECHANICAL_ELECTROLYZER.getDefaultState()).atLocal(0, 0, 0)
			.scale(scale)
			.render(graphics);

		float animation = ((Mth.sin(AnimationTickHolder.getRenderTime() / 32f) + 1) / 5) + .5f;

		blockElement(CKPartialModels.MECHANICAL_ELECTROLYZER_POLE).atLocal(0, animation, 0)
			.scale(scale)
			.render(graphics);

		blockElement(CKPartialModels.MECHANICAL_ELECTROLYZER_HEAD).atLocal(0, animation, 0)
			.scale(scale)
			.render(graphics);
