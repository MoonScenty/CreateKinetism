...
> 사라진 레시피 때문에 여덟 개 기계가 지금 할 일이 없음 — 아래 「지금 멈춰 있는 것」 참고.
> 앞으로의 방향은 아직 정하지 않았음.
...
부르던 레시피 26개도 같이 지웠음.** 결과는 아래 「지금 멈춰 있는 것」에 적음.

### 남은 31종 — 석유 계통

이제 이 모드가 직접 등록하는 유체는 Petrochem에서 가져온 석유 계통뿐임. Mekanism에 대응물이
없으므로 그대로 남음.
...
> **지금 안 돌아감.** 강철 주괴는 이제 `mekanism:ingot_steel`이고, 아래 인퓨전 라인은 인퓨전
> 유체가 사라지면서 레시피째 지워졌음. Mekanism 자신의 야금 주입기로 만들면 됨. 아래는 이
> 모드가 원래 어떻게 만들었는지의 기록임.
...
자바 219개 파일, 리소스 JSON 629개, 레시피 130개.

### 지금 멈춰 있는 것

Mekanism 전환으로 레시피 26개가 사라지면서 **레시피가 0개가 된 기계가 여덟 개**임. 블록도
레시피 타입도 JEI 카테고리도 전부 살아 있고, 넣을 레시피만 없음.
...
| 기계식 야금 주입기 (`infusing`) | 레드스톤·탄소 인퓨전 | 〃 |
| 산화조 (`converting`) | 인퓨전 4종 | 〃 |
| 태양 중성자 활성기 (`activating`) | 폴로늄 | 〃 |
| 방사성 폐기물 드럼 (`decaying`) | 사용후 핵폐기물 | 〃 |

**갈림길은 하나임.** Mekanism의 `Chemical`을 Create 파이프가 만질 수 있는 유체로 건네주는 다리를
놓거나(양쪽 API를 다 물어야 함), 아니면 이 14종만 다시 우리 유체로 등록하고 Mekanism의 같은
이름과 공존시키거나. 어느 쪽도 아직 정하지 않았음.
