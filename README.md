...
- 블록 42종, 유체 35종(석유 31종 + 인퓨전 4종), 아이템 8종
...
**기계식 야금 주입기에 아이템 칸이 하나 있음.** 여기에 재료를 넣으면 기계 자기 탱크 안에서
인퓨전 가스로 녹고, 그대로 노즐이 아래로 떨어뜨림. 레시피 타입은 `converting`임.
...
**인퓨전 4종은 그 뒤 되살렸음.** 지운 이유가 파이프로 옮길 수 없다는 것이었는데, 야금 주입기가
자기 탱크 안에서 만들어 자기 노즐로 떨어뜨리게 하면 애초에 옮길 일이 없음. 남은 10종은 여전히
빈자리임 — 아래 「지금 멈춰 있는 것」.

### 남은 35종 — 석유 계통과 인퓨전 4종

이 모드가 직접 등록하는 유체는 이제 석유 계통 31종과 위의 인퓨전 4종임. Petrochem에서 가져온
석유 쪽은 Mekanism에 대응물이 아예 없으므로 그대로 남음.
...
레드스톤 / 석탄 / 다이아몬드      ──[주입기 칸]──> 각 인퓨전 10mB
농축된 레드스톤 등 Enriched 4종   ──[주입기 칸]──> 각 인퓨전 80mB
...
**야금 주입기(`infusing`)와 산화조(`converting`)는 되살렸음.** 인퓨전 유체 4종을 다시 등록하고,
만드는 자리를 주입기 안으로 옮겼음 — 기계 안에서 만들어 기계 안에서 쓰므로 파이프로 옮길 일이
없고, 그러면 Mekanism에 대응 유체가 없다는 사실이 문제가 되지 않음. 위 「인퓨전은 야금 주입기가
스스로 만듦」 참고.
