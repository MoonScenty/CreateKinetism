...
수율 차이가 Mekanism의 농축 비율임 — 평범한 아이템은 Enriched 형태의 8분의 1이라, 농축은
관문이 아니라 효율 업그레이드임.
...
Mekanism의 강철 라인이자 이 모드에 기계식 주입기가 존재하는 이유임. 나오는 것은
`mekanism:enriched_iron`과 `mekanism:dust_steel`로, 재료도 결과도 Mekanism 것임.

```
레드스톤                          ──[산화실]──> 레드스톤 인퓨전 10mB
석탄                              ──[산화실]──> 탄소 인퓨전 10mB

레드스톤 / 석탄 / 다이아몬드      ──[주입기 칸]──> 각 인퓨전 화학물질 10mB
농축된 레드스톤 등 Enriched 4종   ──[주입기 칸]──> 각 인퓨전 화학물질 80mB
