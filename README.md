...
### 레시피는 Mekanism 것을 그대로 읽음

이 기계는 Mekanism의 야금 주입기에 축을 단 것이므로, 그쪽 레시피 서른일곱 장을 베껴 쓰는 것은
같은 데이터를 두 벌 두고 갈라지게 두는 일임. 그래서 안 씀. **`mekanism:metallurgic_infusing`과
`mekanism:chemical_conversion`을 레시피 매니저에서 그대로 읽어 우리 타입으로 감쌈** — 제어 회로
4종, 합금 3종, 청동, 강철, 네더라이트 가루, 정제 흑요석 가루, 이끼 낀 블록들, 흙→균사체/포드졸,
점토→슬라임까지 전부 돎. 팩이 Mekanism에 레시피를 더하면 여기에도 따라 들어옴.

우리 타입으로 쓴 레시피도 그대로 읽고 목록의 앞에 옴 — Mekanism이 안 하는 일을 이 기계만
하게 만들 자리는 남겨 뒀음. 지금 그런 레시피는 한 장도 없음.

거르는 것이 둘 있음. **아이템을 두 개 이상 요구하는 레시피**는 못 씀 — 벨트에서 한 번에 한 개씩
집으므로 나머지를 공짜로 주게 됨. 그리고 **인퓨전이 아닌 화학물질을 만드는 변환**(부싯돌→산소,
황 가루→황산 같은 것)은 안 가져옴. 탱크가 한 번에 한 종류만 담으므로, 아무 주입 레시피도 못 쓰는
것으로 탱크가 막히기 때문임.
...
Mekanism 것임 — 레시피를 그쪽에서 그대로 읽으므로 다를 수가 없음.
...
자바 218개 파일, 리소스 JSON 624개, 레시피 129개.
