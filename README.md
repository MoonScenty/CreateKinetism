...
> **황 수치가 10분의 1로 떨어짐.** 손으로 쓴 옛 레시피는 황 가루 하나에 이산화황 1000mB였는데
> Mekanism은 100mB임. 아래 Claus 공정 쪽은 500~1000mB 단위로 짜여 있으므로, **석유에서 오는**
> **경로는 그대로지만 황 가루를 태워 되돌리는 경로는 열 배 느려짐.** Mekanism 수치를 그대로 쓴
> 결과이고, 라인 전체를 Mekanism 단위로 다시 맞출지는 아직 정하지 않았음.
