...
이 모드가 직접 등록하는 유체는 이제 Petrochem에서 가져온 석유 계통뿐임. Mekanism에 대응물이
아예 없으므로 그대로 남음.
...
**가스 10종(LPG·프로판·부탄·천연가스·사워가스·황화수소·질소·휘발성 가스·플라스틱·공기)은
가상 유체**임 — Create의 `VirtualFluid`. 양동이 없음, 유체 블록 없음, 월드에 설치 불가. 파이프로
옮기거나 아니면 못 옮김. 사워가스 한 양동이는 곤란하다는 게 이유임.

### 화학 용기 (Chemical Canister)

가상 유체에는 대가가 하나 있음. `VirtualFluid.getBucket()`이 `Items.AIR`을 **하드코딩해서**
돌려주는데, Create의 필터는 유체를 "그 유체가 담긴 아이템"으로만 지목함. 그래서 가상 유체는
스마트 유체 파이프에 이름을 댈 방법이 아예 없었음.

화학 용기가 그 빠진 아이템임. **양동이가 없는 유체만** 받으므로 물과 기름 21종은 거부되고, 두
용기가 서로를 알 필요가 없음.

> 남은 가상 유체가 10종뿐이라 이 물건의 값어치도 예전만 못함. Mekanism 유체는 전부 진짜 양동이를
> 가지므로 용기가 필요 없음. 탱크·파이프·베이슨에 우클릭하면 채워지고 담긴 채로 우클릭하면
부어짐 — 주입구는 안 씀. 기체를 주입구로 따라 붓는 그림이 애초에 틀렸음.

JEI에는 가스마다 채워진 용기가 따로 올라가 있음. 유체 항목을 직접 클릭해서 꺼내는 것은 JEI
내부 클래스를 건드려야 되는 일이라 안 함 — 검색하면 바로 옆에 용기가 뜸.
