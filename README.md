...
> 사라진 레시피 때문에 네 개 기계가 지금 할 일이 없음 — 아래 「지금 멈춰 있는 것」 참고.
> 앞으로의 방향은 아직 정하지 않았음.
...
자바 217개 파일, 리소스 JSON 805개, 레시피 310개.

### 지금 멈춰 있는 것

Mekanism 전환으로 레시피 26개가 사라지면서 여덟 개 기계가 레시피 0개가 됐음. 그중 **둘은
되살렸고, 용해실과 세척기까지 넷을 되살려 넷이 남았음.** 블록도 레시피 타입도 JEI 카테고리도 전부
살아 있고, 넣을 레시피만 없음.

| 멈춘 기계 | 필요했던 것 | Mekanism 쪽 사정 |
|---|---|---|
| 결정화실 (`crystallizing`) | 깨끗한 슬러리 | `Chemical`로만 있음 |
| 원심분리기 (`centrifuging`) | 폴로늄·플루토늄 | 〃 |
| 태양 중성자 활성기 (`activating`) | 폴로늄 | 〃 |
| 방사성 폐기물 드럼 (`decaying`) | 사용후 핵폐기물 | 〃 |

**용해실과 세척기가 이 표에서 빠졌음.** 화학물질 탱크를 달아 슬러리를 직접 주고받게 했더니 대체할
유체가 없다는 사실이 문제가 아니게 됨. 이제 5배 체인이 원석 → 더러운 슬러리 → 깨끗한 슬러리까지
이어지고, **결정화실 하나만 고치면 5배가 끝까지 돎.**
...
**남은 넷도 갈림길은 같음.** Mekanism의 `Chemical`을 Create 파이프가 만질 유체로 건네주는
