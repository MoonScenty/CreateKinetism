...
- 블록 43종, 유체 60종, 아이템 52종
- **자체 광물 Kinetite.** Mekanism의 오스뮴 자리를 대신하는 주황빛 금속. 여기서 메커니즘 4종·
  합금 3종·Enriched 4종의 상위 티어 부품 라인이 갈라져 나옴
- **Curios 아이템 4종.** Mekanism의 MekaSuit 자리를 갑옷 대신 부적·등·반지 칸으로 답함 —
  적 탐지기, 회전 겉날개, 회전 태엽, 탐식 진공관. 넷 다 회전 축전기로 충전함
...
Mekanism의 후반부는 **MekaSuit**임. 모듈을 꽂아 능력을 붙이는 동력 갑옷이고, 그걸 그대로 옮기면
갑옷 모델과 모듈 시스템과 전용 에너지 통을 통째로 다시 짓게 됨 — 수송 체계 4개를 유체 하나로
접어 버린 것과 같은 이유로 안 함.

대신 **Curios 칸에 넣음.** Curios는 이 모드의 **필수 의존성**임 — 선택이 아님. 그쪽이 갑옷 슬롯을
건드리지 않으므로 Create의 기존 장비와 부딪히지 않고, 무엇보다 이미 있는 것을 다시 만들지 않는다는
3번 규칙에 그대로 들어맞음.

플레이어에게 **부적(charm)·등(back)·반지(ring)** 세 칸을 줌.

| 아이템 | 칸 | 하는 일 | 충전량 소모 |
|---|---|---|---|
| 적 탐지기 (Enemy Radar) | 부적 | 반경 15칸의 적대 몹에게 발광 1분 | 표시 1마리당 200 |
| 탐식 진공관 (Greed Tube) | 부적 | 영양바를 먹여 허기를 채움 | 1점당 400 |
| 회전 겉날개 (Kinetic Elytra) | 등 | 겉날개. ` 키로 발진·부스트 | 비행 틱당 10 / 부스트 2,000 |
| 회전 태엽 (Kinetic Spring) | 반지 | 착용 중인 다른 충전템을 감아 줌 | — (틱당 32를 넘겨줌) |

**넷 다 같은 저장분을 씀.** 새 화폐를 만들지 않았음 — 회전 축전기가 적립하는 그 단위(응력 × 틱)
그대로이고, 회전 분해기와도 같은 단위임. 용량은 하나당 64,000이고 축전기 위에 올려 두면 틱당
128씩 들어감(= 25초에 하나). 가득 찬 축전기 하나가 열 개를 채움.

### 각각을 그렇게 만든 이유

- **적 탐지기**는 화면에 무언가를 띄우지 않음. HUD도 미니맵도 없고 **월드 자체가 표시기**임 —
  벽 너머로 몹이 빛나는 것이 곧 정보라서, 뛰면서 볼 수 있는 유일한 형태임. 1초마다 훑되 **이미
  발광 중인 몹은 건너뜀.** 그래서 치운 방에 서 있으면 한 푼도 안 나가고, 같은 스켈레톤에게 1분에
  스무 번씩 발광을 다시 걸며 저 혼자 바닥나는 일도 없음.
- **회전 겉날개**는 내구도 대신 저장분으로 남. 만충이 320초쯤이고 빈 날개는 아예 안 펴짐
  (`canElytraFly`). 등 칸에 있는데도 날 수 있는 것은 **믹스인 2개** 덕분임 — 아래 믹스인 절 참고.
  ` 키를 누르면 지면에서는 발진, 공중에서는 폭죽과 같은 가속 공식이 적용됨. 소리는 폭죽이 아니라
  삼지창 급류를 쓰고, 발진 쪽에만 무거운 변주를 걸어 두 상황이 귀로 구분됨.
- **회전 태엽**은 나머지를 위한 예비분임. 이 모드의 충전은 전부 "축전기 블록 위에 올려놓기"인데,
  그건 거점에서 쓰는 도구에나 맞는 방식이고 몸에 걸치고 나가는 장비에는 틀렸음. **한 번에 하나씩만
  먹임** — 셋을 동시에 반쯤 채워 봐야 셋 다 못 쓰는 상태가 되지만, 차례로 채우면 첫 번째는 당장
  쓸 수 있음. 자기 자신은 동일성으로 걸러내므로 태엽 두 개는 그냥 더 큰 예비분임.
- **탐식 진공관**은 영양바가 음식이 아닌 이유임. 영양바 혼합기가 손실 없이 찍어내는 그 바는
  우클릭해도 아무 일이 없고, 이 물건만이 그걸 씀. 허기가 **1점이라도** 비는 순간 소지품에서 한
  개를 꺼내 태우고 그 1점을 채우며 음료 마시는 소리를 냄. 1초에 한 입이라 빈 허기가 다 차는 데
  20초 걸림 — 한 틱에 스무 개를 삼키고 소리가 스무 겹으로 겹치는 것을 막으려는 것이고, 보급받는
  느낌으로 읽히기도 함. 만충 진공관이 여덟 끼분임. 텍스쳐는 Create의 전자 진공관을 궁극 메커니즘의
  보라색 램프로 리맵한 것임.

Curios 아이템에는 **능력 등록 코드가 없음.** `ICurioItem`을 구현한 Item에는 Curios가
`CuriosCapability.ITEM`을 알아서 붙임. 칸을 나눠주는 것은 `data/createkinetism/curios/entities/`
쪽이고, 어느 아이템이 어느 칸에 들어가는지는 `data/curios/tags/item/` 쪽임 — 양쪽 다 데이터라서
팩이 덮어쓸 수 있음.
...
대부분은 Create의 `VirtualFluid` — 즉 **가상 유체**임. 양동이 아이템 없음, 유체 블록 없음, 월드에
설치 불가. 파이프로 옮기거나 아니면 못 옮김. 염소 한 양동이는 곤란하고, 유체 58종에 전부 양동이를
주면 아이템 목록이 그만큼 늘어남.

**예외는 석유 계통의 액체 23종임.** 원유·정제유·잔사유 2종·유정 염수·탈염유, 중유·경유 4종,
가스유 3종, 등유 2종, 나프타 3종, 휘발유 3종, 윤활유 — 이것들은 **양동이와 유체 블록을 가짐.**
따라 담아 옮길 수 있고, 다른 모드와 주고받을 수 있고, 월드에 부을 수도 있음. 실제로 부을 만한
물건이라는 게 이유임. 같은 계통이라도 **가스(LPG·프로판·부탄·천연가스·사워가스·황화수소·에틸렌·
질소·휘발성 가스)와 증기·공기는 가상 유체 그대로**임 — 위에 적은 이유가 그대로 적용됨.

### 화학 용기 (Chemical Canister)

가상 유체에는 대가가 하나 있음. `VirtualFluid.getBucket()`이 `Items.AIR`을 **하드코딩해서**
돌려주는데, Create의 필터는 유체를 "그 유체가 담긴 아이템"으로만 지목함. 그래서 가스 35종은
스마트 유체 파이프에 이름을 댈 방법이 아예 없었음.

화학 용기가 그 빠진 아이템임. **양동이가 없는 유체만** 받으므로 물과 기름 23종은 거부되고, 두
용기가 서로를 알 필요가 없음. 탱크·파이프·베이슨에 우클릭하면 채워지고 담긴 채로 우클릭하면
...
### 믹스인 4개 — 회전 축전기와 겉날개 때문에

이 모드에는 오랫동안 믹스인이 하나도 없었음. 회전 축전기를 만들면서 처음 두 개가 생겼고 회전
겉날개가 두 개를 더 붙였음. 두 쌍이 각각 한 가지 이유를 공유함.
...
**나머지 둘은 등 칸의 겉날개가 날게 하려는 것임.** 바닐라의 활공 판정 두 곳
(`LivingEntity.updateFallFlying`, `Player.tryToStartFallFlying`)이 **가슴 칸 하나만** 읽음.

```java
ItemStack itemstack = this.getItemBySlot(EquipmentSlot.CHEST);
```

NeoForge가 얹어 준 `canElytraFly` / `elytraFlightTick` 훅도 "가슴 칸의 ItemStack"이라고 문서에
못박아 놨으므로 장신구가 밖에서 끼어들 자리가 없음. 그래서 **그 조회 한 줄만 `@Redirect`** 함 —
가슴 칸이 날 수 있으면 그쪽이 이기고, 못 날 때만 등에 걸친 것을 내놓음. 그 아래는 훅 둘을 포함해
전부 우리 스택으로 그대로 돎.

Curios 틱에서 활공 플래그를 손으로 켜 두는 방법도 있었지만 안 썼음. 우리 틱이 바닐라 틱 뒤에
오는 동안만 동작하고, 순서가 바뀌면 아무 소리 없이 깨짐.
...
- **레시피 없는 아이템.** 메커니즘 4종과 합금 3종, 그리고 Curios 아이템 4종은 아이템과 텍스처만
  있고 만들 방법이 없음.
...
- **겉날개 모델.** 등 칸의 날개가 뒤쪽으로 튀어나오므로 벽에 붙어 서면 블록을 뚫고 보임.
  바닐라 겉날개도 같은 모양이지만 그쪽은 갑옷 렌더러가 처리해 주는 자리임.
- **EMI.** 연동 없음. JEI만 지원함.
...
| ~~Seismic Reader~~ | 손에 든 채 아래 광층을 읽는 것뿐이라 안 만들기로 함. **적 탐지기**가 그 자리를 대신 가져감 — 범위 밖 |
...
| **Modification Station** | 메카슈트 모듈용. 우리는 Curios로 갔으므로 자리가 없음 — 범위 밖 (「Curios」 절 참고) |
...
안 만들기로 함. 모듈 갑옷을 옮기면 갑옷 모델, 모듈 시스템, 전용 에너지 통을 통째로 다시 짓게
되고, 그건 수송 체계 4개를 유체 하나로 접어 버린 것과 정확히 같은 종류의 중복임. 같은 힘을
**Curios 장신구**로 대신 냄 — 위 「Curios」 절.
